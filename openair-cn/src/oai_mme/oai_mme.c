  ...
  char *argv[])
{
  ...
  // MessageDef *arti = (MessageDef *)malloc(sizeof(MessageDef));
  // for(ITTI_MSG_ID(arti) = 0; ITTI_MSG_ID(arti)<100; ITTI_MSG_ID(arti)++)
  // printf("%s = %d\n", itti_get_message_name(ITTI_MSG_ID(arti)), ITTI_MSG_ID(arti));

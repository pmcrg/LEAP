  ...
  nas_message_decode_status_t   * decode_status)
{
  ...
  case ATTACH_COMPLETE:
    ...
    // leap_wait_command();
    // leap_send(assocfd, LEAP_WAIT_COMMAND, "", 0);
    break;
...
static int _emm_as_send (const emm_as_t * msg)
{
  ...
  case _EMMAS_SECURITY_REQ:
    ...
    // for security mode command test // leap_send(assocfd, LEAP_SEC_MODE_COMMAND_NAS_DATA, *(&as_msg.msg.dl_info_transfer_req.nas_msg->data), blength(as_msg.msg.dl_info_transfer_req.nas_msg));
...
static int _emm_as_security_req (const emm_as_security_t * msg, dl_info_transfer_req_t * as_msg)
{
  ...
  if (size > 0) {
     ...
     */
    /*  TODO FOR leap
    if msg.type == 93 //security_mode_command
      send
    
    host:
      recv
      is_msg_type_93
      send back and call at some time
      //in tcpserver.c:
        copy emm_as.c:
        1328-1361
    */
